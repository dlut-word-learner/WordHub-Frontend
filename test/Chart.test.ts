...
import { concatDate, getDateStringFromToday } from "../src/components/Chart/Chart";
...
describe('concatDate', () => {
    it('should return an array of tuples with date and number', () => {
        ...
        const expected = [[beforeYesterday.toISOString().slice(0, 10), 1], [yesterday.toISOString().slice(0, 10), 3], [today.toISOString().slice(0, 10), 2]];
        expect(result).toEqual(expected);
    });
});